...
import com.payflow.app.util.AccountNumberGenerator;
...
public class OrganizationServiceImpl implements OrganizationService {
	...
	private OrganizationResponse toResponse(Organization org) {
		...
		if (org.getBankAccounts() != null && !org.getBankAccounts().isEmpty()) {
			org.getBankAccounts().stream().filter(acc -> "ACTIVE".equals(acc.getStatus())).findFirst().ifPresent(
					acc -> res.setBankAccount(BankAccountResponseDTO.builder().accountNumber(acc.getAccountNumberEnc())
							.ifsc(acc.getIfsc()).status(acc.getStatus()).build()));
		}
	...
	public OrganizationResponse registerOrganization(CreateOrganizationRequest req, List<MultipartFile> documents)
		...
		String accountNum = accountNumberGenerator.generateUniqueAccountNumber();

		// Handle bank account if provided
		BankAccountRequestDTO bankReq = req.getBankAccount();
		BankAccount bankAccount = BankAccount.builder().organization(org).ownerType(Role.ORG_ADMIN)
				.accountNumberEnc(accountNum).status("ACTIVE").ifsc("PAYF0123456").build();
	...
	public OrganizationResponse updateOrganization(Long id, UpdateOrganizationRequest req) {
		...
		if (bankReq != null) {
			...
			BankAccount bankAccount = BankAccount.builder().organization(org).ownerType(Role.ORG_ADMIN)
					.accountNumberEnc(bankReq.getAccountNumber()).ifsc(bankReq.getIfsc()).build();

...
public interface DepositRequestRepository extends JpaRepository<DepositRequest, Long> {
    ...
    List<DepositRequest> findByClientId(Long clientId);

    Page<DepositRequest> findByStatus(DepositStatus status, Pageable pageable);

    Page<DepositRequest> findByStatusAndCreatedAtBetween(DepositStatus status, LocalDateTime start, LocalDateTime end, Pageable pageable);
